...
from engine.data.utils_data import (
    clean_text,
    parse_year_from_datestr,
...
def filter_for_quality(df: DataFrame, min_abstract_len: int = 40) -> DataFrame:
    return (
        df.where(F.col("arxiv_id").isNotNull() & (F.col("arxiv_id") != ""))
        .where(F.col("title_clean").isNotNull() & (F.length("title_clean") > 0))
        .where(
            F.col("abstract_clean").isNotNull()
            & (F.col("abstract_len") >= min_abstract_len)
        )
    )
...
def add_eda_helpers(df: DataFrame) -> DataFrame:
    return df.withColumn(
        "has_doi", (F.col("doi").isNotNull()) & (F.col("doi") != "")
    )


def transform_all(df: DataFrame, min_abstract_len: int = 40) -> DataFrame:
    return add_eda_helpers(
        filter_for_quality(select_and_standardize(df), min_abstract_len)
    )
